...
\draw[anchor=north west,text width=8cm] (9.2cm,10.8cm) node {\begin{footnotesize}"Black feathers in the storm, where will we fall?\\
	Will we ever be home?\\
	Let's let the wind decide, where we are going to fall\\
	Soaring to a destination\\
	She takes me away"\\
	\begin{flushright}- Black Feathers from Black Anima by Lacuna Coil\\(2019).\end{flushright}\end{footnotesize}};
\end{tikzpicture}
