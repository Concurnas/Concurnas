...
\subsubsection{Top level state}
Variables defined outside of a function, class or actor (termed top level state) are copied in a special manner when the are referenced within the context of isolates. All state defined at the top level of a file specifying an isolate and any dependant state defined at top level of dependant file referenced by that isolate is copied into it. Note that top level code is only ever initialized once and the state copied to that isolate is the initial post initialization state.
...
Above we see that \lstinline{fromLib} has 'reverted' to it's initial state of \lstinline{100}, whereas \lstinline{m} is captured as \lstinline{101}. Furthermore, \lstinline{initFromLit()} will be executed only once - so \lstinline{'initlaize fromLib'} will be output to the console once.
...
\begin{itemize}
	...
	\item \lstinline{def hasException() boolean} - returns true if a ref has an exception set on it (non blocking method).
\end{itemize}
...
aref int: = 100
bref int: = 200
assert aref <> bref  // value s not equal
