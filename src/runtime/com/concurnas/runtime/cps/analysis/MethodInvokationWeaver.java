...
import static com.concurnas.runtime.cps.Constants.ALOAD_0;
import static com.concurnas.runtime.cps.Constants.ASTORE_0;
import static com.concurnas.runtime.cps.Constants.DLOAD_0;
import static com.concurnas.runtime.cps.Constants.DSTORE_0;
...
import static com.concurnas.runtime.cps.Constants.D_STATE;
import static com.concurnas.runtime.cps.Constants.D_UNDEFINED;
import static com.concurnas.runtime.cps.Constants.D_VOID;
import static com.concurnas.runtime.cps.Constants.FIBER_CLASS;
import static com.concurnas.runtime.cps.Constants.FLOAD_0;
import static com.concurnas.runtime.cps.Constants.FSTORE_0;
import static com.concurnas.runtime.cps.Constants.ILOAD_0;
import static com.concurnas.runtime.cps.Constants.ISTORE_0;
import static com.concurnas.runtime.cps.Constants.LLOAD;
import static com.concurnas.runtime.cps.Constants.LLOAD_0;
import static com.concurnas.runtime.cps.Constants.LSTORE_0;
import static com.concurnas.runtime.cps.Constants.STATE_CLASS;
import static com.concurnas.runtime.cps.analysis.VMType.TOBJECT;
import static com.concurnas.runtime.cps.analysis.VMType.loadVar;
import static com.concurnas.runtime.cps.analysis.VMType.storeVar;
import static com.concurnas.runtime.cps.analysis.VMType.toVmType;
 ...
 * <p>
 * 
 * Also produces for case where fiber cannot be obtained from stack (e.g. overridden primordial method, e.g. toString
 * ) and must be obtained via getstatic invokation on Iso
 * 
...
public class MethodInvokationWeaver implements Opcodes {
    /**
    ...
    public MethodInvokationWeaver(MethodWeaver mw, BasicBlock aBB) {
        methodWeaver = mw;
        bb = aBB;
...
//            if (u.isLiveIn(i)) {
       ...
       // System.err.println("numBottom: " + numBottom + " getStackLen: " + getStackLen() +" getNumArgs: " + getNumArgs());
    ...
    private static HashSet<String> power4 = new HashSet<String>();
	static {
		power4.add("equals");
		power4.add("toString");
		power4.add("hashCode");
    ...
    void genCall(MethodVisitor mv) {
        ...
        if(mi.owner.contains("[")){//it's an array, we cannot interact with these as not possible to route via CObject
        	loadVar(mv, TOBJECT, methodWeaver.getFiberVar());
            //mv.visitLdcInsn(this.bb.flow.name);
            //mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "(Ljava/lang/String;)" + D_FIBER, false);
        	mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "()" + D_FIBER, false);
            mv.visitInsn(POP);
        	mi.accept(mv);
        }
        else if (mi.desc.indexOf(fiberArg) == -1) { 
        	...
        	boolean isObject = mi.owner.equals("java/lang/Object");
            
            if(isObject && power4.contains(mi.name)){//called on object and is toString, hashcode or equals functions 
            	boolean isEQ = mi.name.equals("equals");//eq handled seperately - for eq we need to switch off the argument from the stack do comparison then switch it back again
            	//special logic required for power 3
        		...
        		mv.visitInsn(DUP);
        		mv.visitTypeInsn(INSTANCEOF, "com/concurnas/bootstrap/runtime/cps/CObject");
        		Label notCobjectTryObj = new Label();
        		mv.visitJumpInsn(IFEQ, notCobjectTryObj);
        		Label l2 = new Label();
        		mv.visitLabel(l2);
        		//is coobject?
        		if(mi.getOpcode() != Opcodes.INVOKESPECIAL ){//|| (!mi.name.equals("toString") && !mi.name.equals("hashCode"))){//dunno why but doesnt work on toString and hashcode
        			//clone and equals ok i guess
        			//TODO: test clone invokation via superclass
                ...
                String befdescBefore = mi.desc;
                String ownerBefore = mi.owner;
                if(!this.bb.flow.classFlow.name.equals("com/concurnas/bootstrap/runtime/cps/CObject")){
                	mi.desc = mi.desc.replace(")", fiberArg);
                	mi.owner = "com/concurnas/bootstrap/runtime/cps/CObject";
                	
                	//if( mi.name.equals("toString")){

                    	//mi.opcode = Opcodes.INVOKEVIRTUAL;
                	//}
                	
                }
                else{
                	mv.visitInsn(POP);//pop off the fiber if its a thing in a CObject
                }
                mi.accept(mv);
                mi.desc = befdescBefore;//restore
                mi.owner = ownerBefore;
                
                
        		Label l3 = new Label();
        		mv.visitLabel(l3);
        		mv.visitJumpInsn(GOTO, complete);
        		mv.visitLabel(notCobjectTryObj);
        		
        		if(isEQ){
        			mv.visitInsn(SWAP);
        		}
        		
        		//its an object or array, so revert to normal call
        		loadVar(mv, TOBJECT, methodWeaver.getFiberVar());
                //mv.visitLdcInsn(this.bb.flow.name);
                //mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "(Ljava/lang/String;)" + D_FIBER, false);
            	mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "()" + D_FIBER, false);
                mv.visitInsn(POP);
                mi.accept(mv);//unaugmented
        		
        		mv.visitLabel(complete);
            }else if(isObject || isfinalObjectMethod(mi)){
            	//e.g. getClass which is final so and cannot have fiberized version
            	loadVar(mv, TOBJECT, methodWeaver.getFiberVar());
                //mv.visitLdcInsn(this.bb.flow.name);
                //mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "(Ljava/lang/String;)" + D_FIBER, false);
            	mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "()" + D_FIBER, false);
            	...
            	//calling a normal fiberized function
            	loadVar(mv, TOBJECT, methodWeaver.getFiberVar());
                //mv.visitLdcInsn(this.bb.flow.name);
                //mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "(Ljava/lang/String;)" + D_FIBER, false);
            	mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "()" + D_FIBER, false);
                ...
                String before = mi.desc;
                if(!this.bb.flow.classFlow.name.equals("com/concurnas/bootstrap/runtime/cps/CObject")){
                	mi.desc = mi.desc.replace(")", fiberArg);
                }
                mi.accept(mv);
                mi.desc = before;//restore
            }
        }
        else{
        	//function which alraedy has fiber argument...
        	loadVar(mv, TOBJECT, methodWeaver.getFiberVar());
            //mv.visitLdcInsn(this.bb.flow.name);
            //mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "(Ljava/lang/String;)" + D_FIBER, false);
        	mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "down", "()" + D_FIBER, false);
