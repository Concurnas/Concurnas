...
import static com.concurnas.runtime.cps.Constants.D_RETURN_ADDRESS;
import static com.concurnas.runtime.cps.Constants.TASK_CLASS;
import static com.concurnas.runtime.cps.Constants.THROWABLE_CLASS;
...
public class BasicBlock implements Comparable<BasicBlock> {
     ...
     */
    public static final int              PAUSABLE           = 1 << 4;
    /*
     ...
     */
    public FixedSizeFrame                  startFrame;
	...
	public boolean hasInvokeDynamicCall = false;
	
	public boolean isMethodCall(){
		//if(forceNoFibering){ return false; } 
		//return false;
		return isMethodCall;
	}
            ...
            switch (opcode) {
                ...
                case INVOKESPECIAL:
                	...
                	if(ain instanceof MethodInsnNode){
                        MethodInsnNode minst = (MethodInsnNode) ain;
                        name = minst.name;
                        owner = minst.owner;
                	}else {

                		InvokeDynamicInsnNode invDynamic = (InvokeDynamicInsnNode) ain;
                        name = invDynamic.name;
                        owner = invDynamic.bsm.getOwner();//invDynamic.
                	}
                	
                	
                    LabelNode il = flow.getOrCreateLabelAtPos(pos);
                    //i.e. set next to true if it's possible to pass the fiber via the stack (as opposed to method argument as per normal [note, u dont have to actually obtain the fiber via this meachnism])
                    boolean isMethodCallViaCallStack = !name.endsWith("<init>");
                    
                    if (pos == startPos) {
                    	this.isMethodCall  = isMethodCallViaCallStack;
                    	if (flow.isPausableMethodInsn(name, owner)) { 
                    		setFlag(PAUSABLE);
                    	}
                    } else {
                        bb = flow.getOrCreateBasicBlock(il);
                        bb.isMethodCall = isMethodCallViaCallStack;
                        if (flow.isPausableMethodInsn(name, owner)) { 
                        	bb.setFlag(PAUSABLE); 
                        }
                        addSuccessor(bb);
    ...
    void coalesceTrivialFollowers() {
        while (true) {
            if (successors.size() == 1) {
               ...
               // boolean ismeth = succ.isMethodCall;
                //System.err.println("" + this.flow.name);
                if (succ.numPredecessors == 1 && lastInstruction() != GOTO && lastInstruction() != JSR && !succ.isPausable() && !succ.isMethodCall ) {
    ...
    //private static final boolean forceNoFibering = false;//TODO: comment this debug logic out
    ...
    void interpret() {
    	try{
    		//Value v, v1, v2, v3, v4;
            FixedSizeFrame frame = startFrame.dup();
            if (isCatchHandler()) {
           ...
           // @SuppressWarnings("unused")
            ...
            //try {
                for (i = startPos; i <= endPos; i++) {
                    ...
                    if(!BytecodeStackFrameModifier.intepret(i, ain.getOpcode(), ain, frame, this.flow.name, this) ){
                    	propagateFrame=false;
                    }
                    //System.err.println(this.flow.name +"[" + this.flow.desc + "]" +  " opcode " + BytecodeOutputter.opcodeToBytecode.get(ain.getOpcode()));
                    /*
                    int opcode = ain.getOpcode();
                    int val, var;
                    //System.err.println(this.flow.name +"[" + this.flow.desc + "]" +  " opcode " + BytecodeOutputter.opcodeToBytecode.get(opcode));
                    ...
                    switch (opcode) {
                        ...
                        case DUP_X2:
                            ...
                            throw new InternalError("Illegal use of DUP_X2 in " + this.flow.name);
                        ...
                        case RETURN:
                           // canThrowException = true;
                            ...
                            if (frame.stacklen != 0) {
                            	...
                            	for(int n=0; n < frame.stack.length; n++){
                            		Value va = frame.stack[n];
                            		sb.append(va !=null ? va.toString(): "null");
                            		if(n != frame.stack.length-1){
                            			sb.append(", ");
                            		}
                            	}
                            	
                                throw new RuntimeException("Expceted stack to be null upon invocation on return opcode but was not in method: '" + this.flow.name + "' stack: " + sb);
                                
                            	//TODO: either insert pops to clean up the above, or just accept it and who cares if the stack is a little dirty? - PARANOID MODE?
                            }
                            break;
                        ...
                        case INVOKESTATIC:
                        case INVOKEINTERFACE:
                        case INVOKEDYNAMIC:	//TODO: check invoke dynamic support is correct
                            ...
                            if (flow.isPausableMethodInsn(min) && frame.numMonitorsActive > 0) {
                                throw new CPSException("Error: Can not call pausable nethods from within a synchronized block\n" +
                                        "Caller: " + this.flow.classFlow.name.replace('/', '.') + "." + this.flow.name + this.flow.desc +
                                        "\nCallee: " + ((MethodInsnNode)ain).name); //TODO: remove this, we dont have synchronized blocks (except in java call?)
                            }
                            //canThrowException = true;
                        ...
                        case MULTIANEWARRAY:
                            ...
                            break;
                        default:
                        	int opcodea = ain.getOpcode();
                            throw new RuntimeException(String.format("Unexpected opcode: '%s' - %s", opcodea, BytecodeOutputter.opcodeToBytecode.get(opcodea)) );
                    }*/
                }
    		...
    		//	e.printStackTrace();
    		RuntimeException t = new RuntimeException("Failure during cps transform of '" + this.flow.name +  this.flow.desc + "' of '" + this.flow.classFlow.name + "': " + e.getMessage(), e);
 ...
 */
    ...
    void mergeSuccessors(FixedSizeFrame frame) {
     ...
     * @param localsOnly
     */
    void merge(FixedSizeFrame inframe, boolean localsOnly) {
     ...
     * 
     */
    ArrayList<BasicBlock> inline() throws CPSException {
    ...
    static ArrayList<BasicBlock> dupCopyContents(boolean deepCopy,
            ...
            HashMap<BasicBlock, BasicBlock> bbCopyMap,
            HashMap<LabelNode, LabelNode> labelCopyMap) throws CPSException {
        ...
        for (BasicBlock orig : targetBB.getSubBlocks()) {
            ...
            dup.isMethodCall = orig.isMethodCall;
     ...
     * linked to it.
     */
    public ArrayList<BasicBlock> getSubBlocks() throws CPSException {
        if (subBlocks == null) {
            ...
            while (!stack.isEmpty()) {
                ...
                for (BasicBlock succ : b.successors) {
                    if (succ == this) {
                        throw new CPSException("JSRs looping back to themselves are not supported");
                    }
     ...
     */
    void checkPausableJSR() throws CPSException {
    ...
    void changeJSR_RET_toGOTOs() throws CPSException {
