...
/**
 * An activation frame - with fixed size
 *
 */
public class FixedSizeFrame extends AbstractFrame {
	public Value[] locals;
    public Value[] stack;
    public int numMonitorsActive = 0;
    public int stacklen = 0;
    
    private FixedSizeFrame(int nLocals, int nStack, boolean init) {
    ...
    public FixedSizeFrame(int nLocals, int nStack) {
     ...
     *      or a local variable
     */
    public FixedSizeFrame merge(FixedSizeFrame inframe, boolean localsOnly, Usage usage) {
        int slen = stacklen;
        ...
        if (nst == null && nlo == null) {
            ...
            return new FixedSizeFrame(nlo, nst, slen, numMonitorsActive);
        }
    }
    ...
    private FixedSizeFrame(Value[] alocals, Value[] astack, int astacklen, int aNumMonitorsActive) {
        this.locals = alocals;
        this.stack = astack;
    ...
    public FixedSizeFrame dup() {
        return new FixedSizeFrame(dupArray(locals), dupArray(stack), stacklen, numMonitorsActive); 
    }
    
    public FixedSizeFrame(String classDesc, MethodNode method) {
    ...
    public Value getLocal(int local, int opcode) {
        ...
        throw new RuntimeException("Expected " + expected + " in local# " + local + ", got " + desc);
    }
    ...
    public boolean equals(Object other) {
        FixedSizeFrame that = (FixedSizeFrame)other;
    ...
    public String stacktoString() {
        ...
        for (int i = 0; i < stsize; i++) {
            sb.append(this.stack[i].getTypeDesc()).append(" ");
        }
        return sb.toString();
    }
}
